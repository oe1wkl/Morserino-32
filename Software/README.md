...
* **Echo Think T.** in the Echo Trainer gave you its thinking time twice: once after the prompt, as intended, and then again after your answer, before the next prompt. Answer quickly, and the Morserino still waited out the rest of the think time before sending the next word — with a long setting, many seconds of silence after every answer. Fixed — the think time is now only a grace period for starting your answer; once you have answered, the next prompt follows just as promptly as with the setting at 0.
* In the Koch Trainer menu, **Preview Char** could only be reached turning the ENCODER one way. Coming from **Learn New Chr** you landed on it, but stepping back from **CW Generator** jumped straight over it to **Learn New Chr**. Fixed. Thanks to Christian, OE1CKO, who found and fixed this, and whose check of every menu link in every firmware variant now runs automatically, so this kind of slip cannot come back unnoticed.
* Coming back from the preferences into the **Echo Trainer** (and into the Koch Trainer's echo modes) left the Morserino unresponsive for five seconds: a touch of the paddle did not resume the exercise, and FN and the ENCODER appeared dead as well, because the top line was showing a result („0 errs (0 wds)") instead of the speed. Leaving the preferences was being treated as the end of a set of words — which is when that result is shown, and held for five seconds so you can read it — although nothing had finished. Fixed: leaving the preferences now simply pauses, with the „continue with paddle" prompt, and the paddle carries on at once.
* M32 Pocket only: the click you hear when turning the ENCODER sometimes had a short crackle in it, through headphones just as through the loudspeaker. The click is made of two very short tones, and the second one was started while the first was still fading out, which cut the sound to silence for an instant; how audible that was depended on the exact moment, so it came and went from one click to the next. Fixed — the second tone now waits until the first has faded out completely. The click is also a little quieter than before: its higher tone lies close to the resonance of the small loudspeaker, which could make it rattle at higher **Tone Volume** settings. Thanks to Noe Gonzalez and Travis Howse, who reported it and sent the recordings that pinned it down.
* In the Echo Trainer, a word with four or more e's in a row could never be answered correctly: the fourth e you keyed was taken as the error sign („eeee" wipes what you have entered so far) and cleared your answer. This showed up most with a **Practice Set** of just „e". Fixed — as long as the e's you key still match the word, they count as letters. Thanks to Christian, OE1CKO.
...
#### Bug Fixes:
* M32 Pocket only: the firmware installer left the previous edition's file system behind when you switched between the Standard and the Accessibility Edition. The two editions divide the internal memory differently and their file systems overlap, so up to 3.6 MB of the old one survived untouched - invisible while the new edition ran, but able to come back to life later and make an Accessibility Edition speak from voice clips it was never built for. The installer now clears that space whenever the edition changes. Your settings, snapshots and scores are unaffected; they are stored elsewhere.
* M32 Pocket only: headphones plugged in while the Morserino was running were often not noticed, so the sound kept coming from the loudspeaker until you unplugged them and plugged them in again. Headphones already in the socket when you switched on were always detected correctly. Which headphones were affected looked random, but it actually depended on how their plug bounced in the socket while being inserted. Fixed.
* M32 Pocket only: several faults on the line-out and headphone outputs. With a line-out setting selected, only the left channel was really switched to line-out - the right one carried on working as a headphone output. The line-out level also depended on whether headphones had happened to be plugged in earlier in the same session. And once any line-out setting had been used, headphones went on being driven in line-out mode until the next restart, which made them sound dull and mushy, most noticeably at lower volume settings. All fixed. One of these was a wrong bit in the codec library - thanks to Harald Klein, the library's author, for merging the fix and releasing it the same day.
...
* **Ultimatic** again, from the same source: the mode used to borrow the Iambic-B timing machinery, and that caused two further faults. A quick tap of the second paddle was thrown away entirely if it fell in the first part of the element in progress, so a deliberate squeeze simply did not appear; and letting go of both paddles at certain moments spliced in a dah that you never asked for - which sounded random, because whether you hit it depended on your release to within a few hundredths of a second. Ultimatic now keeps its own memory of the paddles, as the 1955 design describes: any closure of a paddle, however brief and wherever it falls, is guaranteed to produce its element, and nothing is sent that you are no longer asking for. The **CurtisB DahT%**, **CurtisB DitT%** and **Latency** settings no longer have any effect in Ultimatic - the mode has its own way of reading the paddles and does not need them; contact bounce is dealt with internally. Iambic A, Iambic B and Non-Squeeze are completely unaffected. Again found and precisely analysed by Paweł, SP5DNA, who also went on to test the fix at 40 WpM and noticed that a high Latency setting was still swallowing the occasional deliberate paddle tap - which is why Latency is now out of the picture in Ultimatic altogether.
...
* The Configuration Tool's Files tab showed only the first 30 or so files on the device - the last of them with a size of „NaN KB“ - and the used/free figures at the bottom were missing altogether. The Morserino was assembling that listing in a buffer of fixed size and simply stopped when it was full, without a word about it. Fixed - the listing is now sent in pieces and is complete however many files there are, and it no longer arrives torn over a Bluetooth connection either. Two things in the tool go with it: on the Accessibility Edition, where a complete listing runs to some 500 files, the voice clips are now folded away behind a switch (off by default) so that your own files are not lost among them; and the Device Information panel tells you which edition the connected device is running, Standard or Accessibility - nothing else on the screen could distinguish the two.
* M32 Pocket only: with no cable plugged in - the normal case over Bluetooth - the Configuration Tool reported the power state as „USB - charging“, which was neither true nor useful. Fixed - the Morserino now reports „battery“ when it is running on its own cell, and the tool shows the battery voltage in exactly that case (with a supply connected the voltage sits near 4.2 V straight away and says nothing about the state of charge). The classic M32 is unchanged here: it has no power-good signal and so genuinely cannot tell the two situations apart.
* „Cancel“ in the firmware installer did not really cancel. The Morserino was left sitting in the processor's built-in update mode with a dark screen, doing nothing, until it was unplugged and connected again - which is precisely the confusion Cancel is there to avoid. Fixed - Cancel, and „Install something else“ with it, now restarts the device into the firmware it already has. Two related things: the line telling you what is installed at the moment now names the edition (Standard or Accessibility) as well as the version, and the manual explains why the screen goes dark as soon as the browser connects - at that point no firmware is running at all, only the update mode built into the processor.
* The FN button and the ENCODER knob were dead for about a second between repetitions in the Echo Trainer, in Koch **Learn New Chr** and in the Koch Echo modes — nearly two seconds at slow speeds. A press that began and ended inside that gap was lost altogether, and one still held when the gap ended was acted on late, so the buttons felt unreliable rather than merely slow. The Morserino was sitting out the pause around the OK/ERR verdict without looking at anything else; it now keeps watching the controls while it waits. The pauses themselves are unchanged, so nothing sounds different.
* Selecting **Custom Chars** in **Koch Sequence** a second time could come back with an empty character set, even though your `player.txt` was perfectly fine. Three separate readers of that file shared one "have we just run past the end" marker between them, and if the File Player happened to have stopped exactly at the end, the scan for the custom characters saw that marker on its very first read and gave up immediately. The same re-selection also threw away the File Player's saved reading position — permanently, since that position is written to memory when you leave the preferences — so the next time you opened the File Player it started again from the first word. Both fixed. Thanks to Christian, OE1CKO, for finding these and for the fixes.
* Changing speed or volume while the CW Generator or the Echo Trainer was paused left the top line garbled, e.g. `Con(15)60wWpM paddle`. Writing a small field like the speed only ever repainted its own few characters, so the rest of the wider "Continue with paddle" message stayed behind it. The whole line is now repainted in that situation. And because that clears the message — leaving nothing on screen to say the device is paused — the prompt now comes back on its own after ten quiet seconds. Thanks to Christian, OE1CKO.
* Scrolling onto **Custom Chars** with nothing to load briefly showed "No custom set" and then jumped to the next sequence, leaving a stray character from the previously shown option next to the message. The stray character is gone, and the jump is no longer silent: the Morserino falls back to the **M32** sequence — it will not leave the menu claiming Custom Chars while it is really sending something else — and now says which sequence it fell back to, long enough to read. The Accessibility Edition speaks both messages, where before it said nothing at all and then simply announced the new value. Thanks to Christian, OE1CKO.
* Leaving the preferences with a long press of the ENCODER knob also threw you out of whatever you had been doing. Open the preferences from the CW Generator, change a setting, long-press to leave — and instead of returning to the CW Generator you were back at the top menu, with the mode gone. The long press was being counted twice: once to close the preferences, and then again, a fraction of a second later, by the mode underneath, which reads that same gesture as "leave this mode". Fixed. Thanks to Christian, OE1CKO.
* The same double-counted long press in two more places. Leaving a mode with a long press of the ENCODER knob could land you several menu levels above the mode you had left, instead of on it; and a long press inside the menu itself sometimes climbed two levels rather than one, so that from Koch Trainer::CW Generator::Random you arrived at Koch Trainer. Both fixed. Thanks to Christian, OE1CKO.
* Opening the preferences from a running mode with a double click and leaving them again left the scrolling part of the screen empty — only the top line came back. What had been there reappeared only when you resumed with the paddle, and then briefly alongside the new output. The screen was being cleared without anything painting back the lines the Morserino still had in its scrollback. Fixed. Thanks to Christian, OE1CKO.
* The Morserino seemed to freeze for five seconds at the end of a set of words in the Echo Trainer. With **Max # of Words** set to a number rather than *Unlimited*, the M32 closes each set by sending „+“ — the prosign <ar>, „end of message“ — and then shows how many errors you made; for those five seconds it stopped looking at its buttons altogether. FN and the ENCODER were dead, and a press that both began and ended within that time was lost completely, so it felt as though the device had stopped rather than merely paused. The pause itself is unchanged, since it is there to give you time to read the result, but the controls work throughout it now. Thanks to Rodjo, HB9FUR, who ran into this and asked whether the „+“ was a setting he had overlooked — it was (that is **Max # of Words** doing its job), but the frozen five seconds next to it was not.
* M32 Pocket only: the German umlauts were drawn as an empty rectangle in the scrolling display area whenever **Font Size** was set to **Small**. They come up more often than you might expect: „.-.-“ is a perfectly good **ä**, and it is exactly what you get when you key „+“ („.-.-.“) one element short, so anyone practising prosigns in the Echo Trainer meets it sooner or later — and a rectangle tells you nothing about what you actually sent. The characters were never missing from the font: the small font had been restricted to plain ASCII to keep the lines tight enough to fit five of them on the screen, and anything outside that range was drawn as a placeholder box. It now covers the umlauts — and the „(c)“ of the start-up screen, which had been disappearing the same way — and still shows five lines. **Normal** was never affected. Along with it: with **Output Case** set to UPPER the umlauts stayed lower case while every letter around them was capitalised; ä ö ü now become Ä Ö Ü as they should.
