...
void Goertzel::setup() {                 /// pre-compute some values (sine, cosine, coeff, magnitudelimit) that are compute-imntensive and won't change anyway
  goertzel_n = ( MorsePreferences::pliste[posGoertzelBandwidth].value == 0 ? 152 : 608);                 // update Goertzel parameters depending on chosen bandwidth
  magnitudelimit_low = ( MorsePreferences::pliste[posGoertzelBandwidth].value? 160000 : 40000);          // values found by experimenting
