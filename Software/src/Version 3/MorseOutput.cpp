...
void MorseOutput::drawVolumeCtrl (boolean inverse, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t volume) {
  ...
  Heltec.display -> fillRect(x + 2, y + 4, (width - 4) * volume / 100, height - 8);
...
void MorseOutput::pwmTone(unsigned int frequency, unsigned int volume, boolean lineOut) { // frequency in Hertz, volume in range 0 - 100; we use 10 bit resolution
