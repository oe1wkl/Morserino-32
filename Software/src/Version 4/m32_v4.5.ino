...
/*
void printOnDisplay(FONT_ATTRIB style, String s) {   /// now displayGeneratedMorse
  //DEBUG("printOnDisplay");
   MorseOutput::printToScroll(style, s, true, encoderState == scrollMode);
   if (MorsePreferences::serialOut & 0b010)
      Serial.print(s);
}
*/
...
unsigned int wordDoublerICS() {
    if (morseState != morseGenerator || (MorsePreferences::wordDoubler == 0) || ( MorsePreferences::wordDoubler != 0 && firstTime == false))
...
void fetchNewWord() {
  ...
  char numBuffer[16];                // for number to string conversion with sprintf()
    ...
    if (startFirst == true)  {                                 /// do the intial sequence in trainer mode, too
        clearText = "vvvA";
        startFirst = false;
    } else if (morseState == morseGenerator && MorsePreferences::wordDoubler != 0 && firstTime == false) {
...
void SerialOutMorse(String s, uint8_t origin) {
  uint8_t bitmap = (MorsePreferences::serialOut < 5 ? MorsePreferences::serialOut : 7);
  if (origin & bitmap)
      Serial.print(s);
}
...
void updateTimings() {
  ...
  halfICS = (3 + MorsePreferences::interCharSpace) * ditLength / 2;
