...
// the preferences variable and their defaults
  ...
  uint8_t p_serialOut = 1;                    // shall we output characters on USB serial? 0 = none (but DEBUG/ERR) 1 = keyer, 2 = decoder, 3 = both, 7 = everything
...
///// stored in preferences, but not adjustable through preferences menu:
  ...
  uint8_t p_vAdjust = 185;                    // correction value: 175 - 250
...
void displayMorse(boolean decoded) {
  ...
  else if (p_serialOut && (morseState == morseKeyer || morseState == morseTrx || morseState == morseDecoder))
    toBePrinted = true;
  if (toBePrinted)
      Serial.print(symbol);
        ...
        if ((t=checkEncoder())) {
            ...
            switch (pos) {
                ...
                case  posWordLength: p_wordLength += (t+1);                                   // length of English words: 0, or 2-6
                                ...
                                break;
                case  posSerialOut:
                                p_serialOut = (p_serialOut + t + 8) % 8;                      // what to output on USB serial
                                if (p_serialOut == 6)
                                    p_serialOut = 3;
                                else if (p_serialOut == 4)
                                    p_serialOut = 7;
                ...
                case posVAdjust:
                                p_vAdjust += t;
                                p_vAdjust = constrain(p_vAdjust, 165, 254);            
                                displayVAdjust();
                                break;
...
int16_t batteryVoltage() {      /// measure battery voltage and return result sin milliVolts
      ...
      // v *= (2314.35 / counts);
      v *= (p_vAdjust * 12.9 / counts);
      //DEBUG(String(v,2));
      //DEBUG(String(p_vAdjust));
