...
#endif
  // Something wrong with the voice store - no clips at all, or a pack belonging to another
  // firmware - is announced on the sidetone, which is generated by the firmware itself and
  // owes nothing to SPIFFS: the one channel that cannot go missing along with the clips.
  // A no-op when the store is fine. Non-blocking: the same tick() that drives clips runs
  // the tones, and splashPause() below is already calling it, so the alarm fits inside the
  // splash and boot gets no longer.
  MorseVoice::warnClipStore();
  if (!MorseVoice::clipStoreUsable())
      // Nothing there to speak with, so every announcement below would be a silent no-op -
      // and what the operator would meet is a device that says nothing whatsoever,
      // indistinguishable to them from one that failed to boot. The alarm is all we have.
      return;
  // A mismatched pack still holds most of its clips, so we DO go on and speak: the operator
  // gets the alarm and then the usual splash, and only the entries this firmware added
  // since that pack stay silent.
...
#ifdef CONFIG_AUDIO_A11Y
  // Sighted-helper half of the missing-clip warning - the operator already has the alarm
  // tones from announceSplash(). Line 2 is the only free scroll line on this screen
  // (0 = version, 1 = copyright, NoOfVisibleLines-1 = battery), so the fix itself goes to
  // the serial log and the manual rather than on the display. Not voiced, on purpose:
  // see MorseVoice::clipStoreDisplayText().
  if (const char *clipWarning = MorseVoice::clipStoreDisplayText())
      MorseOutput::printOnScroll(2, BOLD, 0, clipWarning, false, true);
#endif
