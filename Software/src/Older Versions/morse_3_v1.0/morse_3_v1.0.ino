...
const int  dutyCycleTwentyPercent = 200;
...
const uint8_t menuN = 34;     // no of menu items +1
...
const uint8_t menuNav [menuN] [5] = {                   // { level, left, right, up, down}
  ...
  {0,_trx,_goToSleep,_dummy,0},                         // 32 decoder
  {0,_decode,_keyer,_dummy,0}                           // 33 goto sleep
};
...
enum prefPos  { posClicks, posPitch, posExtPaddles, posPolarity, 
                ...
                posTrainerDisplay, posWordDoubler, posEchoRepeats,  posEchoConf, 
                posKeyTrainerMode, posLoraTrainerMode, posGoertzelBandwidth, posSpeedAdapt,
                posKochFilter };

const String prefOption[] = { "Encoder Click", "Tone Pitch Hz", "Paddle:      ", "Paddle Polar.", 
                        ...
                        "Trainer Disp ", "Each Word 2x ", "Echo Repeats ", "Confrm. Tone ", 
                        "Key ext TX   ", "Send via LoRa", "Bandwidth    ", "Adaptv. Speed",  
                        "Koch"};                   
 prefPos keyerOptions[] =      {posClicks, posPitch, posExtPaddles, posPolarity, posCurtisMode, posCurtisBDahTiming, posCurtisBDotTiming, posACS };
 prefPos generatorOptions[] =  {posClicks, posPitch, posExtPaddles, posInterWordSpace, posInterCharSpace, posRandomOption, 
                                    posRandomLength, posCallLength, posAbbrevLength, posWordLength, 
                                    posTrainerDisplay, posWordDoubler, posKeyTrainerMode, posLoraTrainerMode };
 prefPos playerOptions[] =     {posClicks, posPitch, posExtPaddles, posInterWordSpace, posInterCharSpace, posTrainerDisplay, posWordDoubler, 
                                     posKeyTrainerMode, posLoraTrainerMode };
 prefPos echoPlayerOptions[] = {posClicks, posPitch, posExtPaddles, posPolarity, posCurtisMode, posCurtisBDahTiming, posCurtisBDotTiming, posACS,
                                    posEchoToneShift, posInterWordSpace, posInterCharSpace, posEchoRepeats,  posEchoConf};
 prefPos echoTrainerOptions[]= {posClicks, posPitch, posExtPaddles, posPolarity, posCurtisMode, posCurtisBDahTiming, posCurtisBDotTiming, posACS,
                                    posEchoToneShift, posInterWordSpace, posInterCharSpace, posRandomOption, 
                                    posRandomLength, posCallLength, posAbbrevLength, posWordLength, posEchoRepeats,  posEchoConf, posSpeedAdapt };
 prefPos kochGenOptions[] =    {posClicks, posPitch, posExtPaddles, posInterWordSpace, posInterCharSpace, 
                                    ...
                                    posTrainerDisplay, posWordDoubler, posKeyTrainerMode, posLoraTrainerMode };
 prefPos kochEchoOptions[] =   {posClicks, posPitch, posExtPaddles, posPolarity, posCurtisMode, posCurtisBDahTiming, posCurtisBDotTiming, posACS,
                                    posEchoToneShift, posInterWordSpace, posInterCharSpace, 
                                    posRandomLength, posAbbrevLength, posWordLength, posEchoRepeats,  posEchoConf, posSpeedAdapt };
 prefPos loraTrxOptions[] =    {posClicks, posPitch, posExtPaddles, posPolarity, posCurtisMode, posCurtisBDahTiming, posCurtisBDotTiming, posACS,
                                    posEchoToneShift };
 prefPos extTrxOptions[] =     {posClicks, posPitch, posExtPaddles, posPolarity, posCurtisMode, posCurtisBDahTiming, posCurtisBDotTiming, posACS,
                                    posEchoToneShift, posGoertzelBandwidth };
 prefPos decoderOptions[] =    {posClicks, posPitch, posGoertzelBandwidth };

 prefPos allOptions[] =        { posClicks, posPitch, posExtPaddles, posPolarity, 
                                    posCurtisMode, posCurtisBDahTiming, posCurtisBDotTiming, posACS, 
                                    posEchoToneShift, posInterWordSpace, posInterCharSpace, posRandomOption, 
                                    posRandomLength, posCallLength, posAbbrevLength, posWordLength, 
                                    posTrainerDisplay, posWordDoubler, posEchoRepeats,  posEchoConf, 
...
// the preferences variable and their defaults
  ...
  uint8_t p_interCharSpace = 3;               // trainer: in dit lengths                          3 - 15
  uint8_t p_tLeft = 20;                       // threshold for left paddle
  uint8_t p_tRight = 20;                      // threshold for right paddle
  boolean p_useExtPaddle = false;             // true if we use an external (mechanical) paddle instead of the touch sensors
  ...
  uint8_t p_responsePause = 5;                // in echoTrainer mode, how long do we wait for response? in interWordSpaces; 2-12, default 4
  ...
  uint8_t p_keyTrainerMode = 1;               // key a transmitter in generator and player mode?
                                              //  0: "Never";  1: "CW Keyer only";  2: "Keyer&Trainer";
...
///// stored in preferences, but not adjustable through preferences menu:
  uint8_t p_wpm = 15;                         // keyer speed in words per minute                  5 - 50
...
const String kochChars = "mkrsuaptlowi.njef0yv,g5/q9zh38b?427c1d6x-=KA+SNV@:";
...
// we MUST reset the OLED RST pin for 50 ms! for the oldboard only
#if BOARDVERSION == 2
...
// check battery, display starup screen
  displayStartUp();
...
////////////  Setup for LoRa
  ...
  pref.begin("morserino", false);                // open namespace "morserino" read-only
 
  /// new code for reading preoferences values - we check if we have a value, and if yes, we use it; if no, we use and write a default value
  ...
  const char * defaultFile = "This is just an initial dummy file for the player. Dies ist nur die anfänglich enthaltene Standarddatei für den Player.\n"
    ...
    delay(2500);  //// just to be able to see the startup screen for a while
...
void displayStartUp() {
  ...
  printOnScroll(1, REGULAR, 0, "© 2018");

  
  printOnScroll(2, REGULAR, 0,  "Battery:");
   ...
   switch (morseState) {
      ...
      case morseGenerator:  if (leftKey  || rightKey)   {                                    // touching a paddle starts and stops the generation of code
                          ...
                          if (active)
                            generateCW();
                          break;
      case echoTrainer:   if (!active && (leftKey  || rightKey))   {                                    // touching both paddles starts and stops the generation of code
                          // for debouncing:
                          while (checkPaddles() )
                              ;                                                           // wait until paddles are released
                          active = !active;
...
/// if we have time check for button presses
    ...
    switch (modeButton.clicks) {                                // actions based on enocder button
       ...
       case 1:    if (morseState == morseGenerator || morseState == echoTrainer) {//  start/stop in trainer modi, in others does nothing currently
              ...
              break;
       case 2:  setupPreferences();                               // double click shows the preferences menu
...
void menu_() {
   ...
   int t;
    ...
    while (true) {                          // we wait for a click (= selection)
        ...
        switch (modeButton.clicks) {                                // actions based on enocder button
          case 2: setupPreferences();                               // all available options when called from top menu
          ...
          case 1: // check if we have a submenu or if we execute the selection
                  ...
                  if (menuNav[newMenuPtr][naviDown] == 0) {
                      ...
                      disp = 0;
                      if (p_menuPtr != _goToSleep) {                          // remember last exectued, unless it is shutdown
                          pref.begin("morserino", false);             // open the namespace as read/write
...
///////////// eGEN_TYPE { RANDOMS, ABBREVS, WORDS, CALLS, MIXED, KOCH_MIXED, KOCH_LEARN };           // the things we can generate in generator mode
  ...
  switch (p_menuPtr) {
    case  _keyer:  /// keyer
     ...
     case _genPlayer:  
                ...
                skipWords(file,p_fileWordPointer);
...
boolean doPaddleIambic (boolean dit, boolean dah) {
  ...
  switch (keyerState) {                                         // this is the keyer state machine
    ...
    /// first we check that we have waited as defined by ACS settings
            ...
            switch ( p_keyermode ) {
              case IAMBICB:  curtistimer = ditLength * p_curtisBDotTiming / 100;   
    ...
    case DAH:
            ...
            ktimer = dahLength;
            switch (p_keyermode) {
              case IAMBICB:  curtistimer = dahLength * p_curtisBTiming / 100;    // enhanced Curtis mode B starts checking after some time
                             break;
              case IAMBICA:
              case ULTIMATIC:
    ...
    case KEY_START:
          ...
          if (morseState == (echoTrainer || loraTrx) && p_echoToneShift != 0) {
    ...
    case INTER_ELEMENT:
            ...
            if (millis() > ktimer) {               // at end of INTER-ELEMENT
                switch(keyerControl) {
                      ...
                      case 7: 
                              if (p_keyermode != ULTIMATIC)        // we are in a Iambic mode
...
boolean checkPaddles() {
  ...
  const int debDelay = 4;       // debounce time = 4 ms
  
  if (p_useExtPaddle) {           // see if paddles shortened to GROUND
    newL = !digitalRead(leftPin);
    newR = !digitalRead(rightPin);
  ...
  if (newR != oldR)
      rTimer = millis();
  if (millis() - lTimer > debDelay)
      if (newL != leftKey) 
          leftKey = newL;
  if (millis() - rTimer > debDelay)
      if (newR != rightKey) 
          rightKey = newR;       
...
void pwmTone(unsigned int frequency, unsigned int volume, boolean lineOut) { // frequency in Hertz, volume in range 0 - 100; we use 10 bit resolution
  const uint16_t vol[] = {0, 2,  40, 180, 290, 400, 500, 620, 750, 900, 1023};
...
void displayTopLine() {
  clearStatusLine();
  printOnStatusLine(true, 0, (p_useExtPaddle ? "X " : "T "));
  if (morseState == morseGenerator) 
    printOnStatusLine(true, 1,  p_wordDoubler ? "x2" : "  ");
  else {
    switch (p_keyermode) {
      case IAMBICA:   printOnStatusLine(false, 2,  "A "); break;          // 0 means we are Iambic A (no paddle eval during dah)
      case IAMBICB:   printOnStatusLine(false, 2,  "B "); break;          // 1: orig Curtis mode: paddle eval during element
      case ULTIMATIC: printOnStatusLine(false, 2,  "U "); break;          // 2: Ultimatic Mode
    }
  }
    ...
    case KEY_UP:
            ...
            if (l==0)  {                                               // fetch a new word if we have an empty word
                if (clearText.length() > 0) {                          // this should not be reached at all.... except when display word by word
                  ...
                  if (morseState == loraTrx || morseState == morseGenerator) {
 ...
 //             // display last character - consider echo mode!
                ...
                if (morseState == echoTrainer) {
                    switch (echoTrainerState) {
                        ...
                        case REPEAT_WORD:
                                          // fall through 
                        case SEND_WORD:   echoTrainerState = GET_ANSWER;
                                          ++repeats;
...
void dispGeneratedChar() {
  ...
  if (generatorMode == KOCH_LEARN || morseState == loraTrx || (p_trainerDisplay == DISPLAY_BY_CHAR 
...
void fetchNewWord() {
  int rssi, rxWpm, rv;
  static boolean firstTime = true;      /// for word doubler mode
    ...
    if (morseState != echoTrainer)  printToScroll(REGULAR, " ");    /// in any case, add a blank after the word on the display
    if (generatorMode == KOCH_LEARN) {
        startFirst = false;
        echoTrainerState = SEND_WORD;
    }
      ...
      randomGenerate:       repeats = 0;
                            switch (generatorMode) {
                                  ...
                                  case KOCH_MIXED:rv = random(3);
                                                  ...
                                                  break;
                                  case PLAYER:    clearText = getWord(file);
...
void setupPreferences() {
  ...
  while (true) {                            // we wait for single click = selection or long click = exit
        ...
        switch (modeButton.clicks) {            // button was clicked
          ...
          exitFromHere: updatePreferences();
                        //delay(200);
                        return;
                        break;
          }
...
void displayExtPaddles() {
     printOnScroll(2, REGULAR, 1,  p_useExtPaddle ? "Ext. Paddle " :
                                                         "Touch Paddle" ); 
}
        ...
        if ((t=checkEncoder())) {
            ...
            switch (pos) {
                case  posCurtisMode : p_keyermode = (p_keyermode + t);                        // set the curtis mode
                                      p_keyermode = constrain(p_keyermode, 1, 3);
                ...
                case  posInterWordSpace : p_interWordSpace += t;                         // interword space in lengths of dit
                                ...
                                updateTimings();
                                break;
                case  posInterCharSpace : p_interCharSpace = constrain(p_interCharSpace + t, 3, 15);  // set Interchar space - 3 - 15 dits
                ...
                case  posEchoConf:  p_echoConf = !p_echoConf;
                                displayEchoConf();
                                break;
                // posLoraTrainerMode, posGoertzelBandwidth, posSpeedAdapt,
...
void updateTimings() {
  ...
  interWordSpace = _max(p_interWordSpace * ditLength, (p_interCharSpace+4)*ditLength);
  effWpm = 60000 / (31 * ditLength + 4 * interCharacterSpace + interWordSpace );  /// we do not use it yet! effective wpm with lengthened spaces = Farnsworth speed
} 

void changeSpeed( int t) {
  p_wpm += t;
  p_wpm = constrain(p_wpm, 5, 50);
  updateTimings();
  displayCWspeed();                     // update display of CW speed
...
int16_t batteryVoltage() {
    ...
    adcAttachPin(batteryPin);
      ...
      const float XS = 1.43766;      //The returned reading is multiplied by this XS to get the battery voltage.
      ...
      delay(100);
      while ( (diff = abs(c - analogRead(batteryPin))) > 75 ) {
        c = analogRead(batteryPin);
        printOnScroll(1, REGULAR, 1, String(diff));
        //Serial.print(".");
        delay(150);
      }
...
#define TEST 0
#if TEST == 1
printOnScroll(2, INVERSE_BOLD, 4, String(v) + " mV");
#else
  int w = constrain(v, 3375, 3900);
  w = map(w, 3375, 3900, 0, 31);
...
void checkShutDown(boolean enforce) {       /// if enforce == true, we shut donw even if there was no time-out
  long int timeOut = ((morseState == loraTrx) || (morseState == morseTrx)) ? 450000 : 300000;  /// 7,5 or 5 minutes
...
///////////// update Preferences in non-volatile storage
...
//// byte 1: header; first two bits are the protocol version (curently 01), plus 6 bit paclet serial number (starting from random)
//// byte 2: first 6 bits are wpm (must be between 5 and 50; values 00 - 04 and 51 to 63 are invalid), the remaining 2 bits are already data payload!
  ...
  int wpm;
    ...
    switch(decoderState) {
      ...
      case INTERCHAR_:    if (checkTone()) {
                              ...
                              lowDuration = millis() - startTimeLow;             // we record the length of the pause
                              lacktime = 5.6;                 ///  when high speeds we have to have a little more pause before new word
                              if (p_wpm > 35) lacktime = 6.6;
                                else if (p_wpm > 30) lacktime = 6.4;
                              if (lowDuration > (lacktime * ditAvg)) {
                                   printToScroll(REGULAR, " ");                       // output a blank                                
                                   decoderState = LOW_;
                              }
...
void keyOut(boolean on,  boolean fromHere, int f, int volume) {                                      
  ...
  //// differentiate eternal (decoder, sometimes cw_generate) and internal (keyer, sometimes Cw-generate) side tones
...
void wifiFunction() {
  ...
  while (true) {                                      // we wait for single click = selection or long click = exit
        ...
        switch (modeButton.clicks) {                  // button was clicked
          case 1:     // execute
                      switch (pos) {
                        case 0:   startAP();          // run as AP to get WiFi credentials from user
                                  break;
                        default:
                        case 1:   uploadFile();       // upload a text file
                                  break;
                        case 2:   updateFirmware();   // run OTA update
                                  break;
                      }
                      break;
          case -1:    //////// long press indicates we are done with setting preferences - check if we need to store some of the preferences
...
}   // end function wifiFunction

void displayWifiMenu( int pos) {
    display.clear();
    printOnStatusLine(true, 0,  "WiFi Functions:");
    printOnScroll(0, pos==0 ? BOLD : REGULAR, 0, String("WiFi Config"));
    printOnScroll(1, pos==1 ? BOLD : REGULAR, 0, String("File Upload"));
    printOnScroll(2, pos==2 ? BOLD : REGULAR, 0, String("Update Firmw."));
}
...
/*  
  server.on("/upload", HTTP_GET, []() {                 // if the client requests the upload page
    if (!handleFileUpload())                // send it if it exists
...
void skipWords(File fn, uint32_t count) {             /// just skip count words in open file fn
  while (count > 0) {
    getWord(fn);
    --count;
  }
