...
enum prefPos  { posClicks, posPitch, posExtPaddles, posPolarity, 
                ...
                posKochSeq, posKochFilter, posLatency, posRandomFile, posTimeOut, posQuickStart, posAutoStop, posLoraSyncW, posMaxSequence,  posSerialOut,
...
const String prefOption[] = { "Encoder Click", "Tone Pitch Hz", "External Pol.", "Paddle Polar.", 
                              ...
                              "Time Out     ", "Quick Start  ", "Auto Stop    ", "LoRa Channel  ", "Max # of Words",  "Serial Output", 
 ...
 prefPos generatorOptions[] =  {posClicks, posPitch, posExtPaddles, posInterWordSpace, posInterCharSpace, posRandomOption, 
                                    ...
                                    posTrainerDisplay, posWordDoubler, posKeyTrainerMode, posLoraTrainerMode, posLoraSyncW, posTimeOut, posQuickStart, posAutoStop, posSerialOut };
 prefPos playerOptions[] =     {posClicks, posPitch, posExtPaddles, posInterWordSpace, posInterCharSpace, posMaxSequence, posTrainerDisplay, 
                                     posRandomFile, posWordDoubler, posKeyTrainerMode, posLoraTrainerMode, posLoraSyncW, posTimeOut, posQuickStart, posAutoStop, posSerialOut };
 ...
 prefPos kochGenOptions[] =    {posClicks, posPitch, posExtPaddles, posInterWordSpace, posInterCharSpace, 
                                    ...
                                    posTrainerDisplay, posWordDoubler, posKeyTrainerMode, posLoraTrainerMode, posLoraSyncW, posKochSeq, posTimeOut, posQuickStart, posAutoStop, posSerialOut };
 ...
 prefPos allOptions[] =        { posClicks, posPitch, posExtPaddles, posPolarity, posLatency,
                                    ...
                                    posTimeOut, posQuickStart, posAutoStop, posSerialOut};
...
// the preferences variable and their defaults
  ...
  uint8_t p_maxSequence = 0;                  // max # of words generated before the Morserino pauses

  uint8_t p_serialOut = 1;                    // shall we output characters on USB serial? 0 = none (but DEBUG/ERR) 1 = keyer, 2 = decoder, 3 = both, 
...
///// stored in preferences, but not adjustable through preferences menu:
  ...
  uint8_t p_vAdjust = 180;                    // correction value: 175 - 250
...
void DEBUG (String s) {
  if (!p_serialOut || IGNORE_SERIALOUT) 
    Serial.println(s);
}
...
/// set up for encoder button
  ...
  /// check if BLACK knob has been pressed on startup - if yes, we have to perform Hardware Configuration
  delay(50);
  if (SPIFFS.begin() && digitalRead(modeButtonPin) == LOW)   {        // BLACK was pressed at start-up - checking for SPIFF so that programming 1st time w/o pull-up shows menu
     ...
     printOnStatusLine(true, 0,  "Release BLACK");
        ...
        ;
     displayKeyerPreferencesMenu(posHwConf);
     adjustKeyerPreference(posHwConf);

     switch (hwConf) {
        case 1: calibrateVoltageMeasurement();
                break;
        case 2: loraSystemSetup();
                break;
        default: break;
     }
...
/*
/// check if paddles were squeezed on start-up - if yes, we have to perform voltage measurement calibration
 checkPaddles(); delay(2);
 checkPaddles();
 DEBUG("Left " + String(leftKey));
 DEBUG("Right " + String(rightKey));
 if (checkPaddles()) {
    delay(100);
    display.clear();
    display.display();
    printOnStatusLine(true, 0, "Calibration...");
    delay(200);
    calibrateVoltageMeasurement();
 }
*/
...
////////////  Setup for LoRa
  ...
  LoRa.setFrequency(p_loraQRG+0000);                       /// default = 434.150 MHz - Region 1 ISM Band, can be changed by system setup
...
/////// System Setup / Battery Measurement Calibration  /////// calles when both paddles are pressed at startup

void calibrateVoltageMeasurement() {
  displayKeyerPreferencesMenu(posVAdjust);
  adjustKeyerPreference(posVAdjust);
  pref.begin("morserino", false);             // open the namespace as read/write
  pref.putUChar("vAdjust", p_vAdjust);
  pref.end();
}
...
// static uint64_t loopC = 0;
   ...
   switch (morseState) {
      ...
      case morseGenerator:  
                          if (p_autoStop) {
                              if ((autoStop == stop1) || leftKey  || rightKey)   {           // touching a paddle starts and stops the generation of code
                              // for debouncing:
                              while (checkPaddles() )
...
boolean menuExec() {                                          // return true if we should  leave menu after execution, true if we should stay in menu
  //DEBUG("Executing menu item " + String(p_menuPtr));
  ...
  switch (keyerState) {                                         // this is the keyer state machine
     case IDLE_STATE:
         ...
         if (millis() > interWordTimer) {
             ...
             //String tmpstr = String(CWtree[treeptr].symb);
             //DEBUG(String(treeptr));
...
boolean checkPaddles() {
  ...
  /* intral and external paddle are now working in parallel - the parameter p_useExtPaddle is used to indicate reverse polarity of external paddle
  */
...
//DEBUG("newL " + String(newL));
//DEBUG("newR " + String(newR));
...
void displayMorse(boolean decoded) {
  String symbol;
  uint8_t toBePrinted = 0;
  symbol.reserve(6);
  if (treeptr == 0)     // we display a blank
    symbol = String(" ");
    //return;
  else
    symbol = CWtree[treeptr].symb;
  //DEBUG("Symbol: " + symbol + " treeptr: " + String(treeptr));
  if (p_serialOut && (morseState == morseKeyer || morseState == morseTrx || morseState == morseDecoder)) {
    toBePrinted = p_serialOut & (decoded? 2 : 1);
    if (toBePrinted)
      Serial.print(symbol);
  }
    ...
    if (morseState == loraTrx) {                                // we check the rxBuffer and see if we received something
       ...
       ////// from here: retrieve next CWword from buffer!
        if (loRaBuReady()) {
            ...
            //DEBUG("Header: " + (String) header);
            //DEBUG("CWword: " + (String) CWword);
            //DEBUG("Speed: " + (String) rxWpm);
            ...
            //DEBUG("clearText: " + (String) clearText);
            //DEBUG("RX Speed: " + (String)rxWpm);
...
void displayKeyerPreferencesMenu(int pos) {
  ...
  if (pos < posLoraBand)
    ...
    printOnStatusLine(true, 0,  "Calibrate Voltage");
  else 
    printOnStatusLine(true, 0,  "Hardware Config. ");
  ...
  switch (pos) {
    ...
    case posAutoStop:     displayAutoStop();
     ...
     case posMaxSequence: displayMaxSequence();
                          break; 
     case posSerialOut:   displaySerialOut();
                          break;
     case posVAdjust:     displayVAdjust();
                          break;  
     case posHwConf:      displayHwConf();
                          break;                  
  } /// switch (pos)
  display.display();
        ...
        if ((t=checkEncoder())) {
            ...
            switch (pos) {
                ...
                case  posWordLength: p_wordLength += (t+1);                                   // length of English words: 0, or 2-6
                                ...
                                break;
                case  posSerialOut:
                                p_serialOut = (p_serialOut + t + 4) % 4;                      // what to output on USB serial
                                displaySerialOut();
                                break;
                case  posMaxSequence: 
                ...
                case posVAdjust:
                                p_vAdjust += t;
                                p_vAdjust = constrain(p_vAdjust, 175, 255          );             // what prompt for echo trainer mode
                                displayVAdjust();
                                break;
...
void echoTrainerEval() {
    ...
    //DEBUG("Response: " + String(echoResponse.length()));
    //DEBUG("Word:     " + String(echoTrainerWord.length()));
...
void printToScroll(FONT_ATTRIB style, String text) {
    // DEBUG("printToScroll" + String(text.c_str()));
...
String SL = "/";
  ...
  if (styleChanged || lengthExceeded) {
    //DEBUG("style/length " + styleChanged + SL + lengthExceeded + SP);
    flushScroll();
  }
  ...
  //DEBUG("AUTO: " + String(printToScroll_autoflush));
  //DEBUG("morseState: " + String(morseState));
  if (morseState != morseGenerator)
    printToScroll_autoflush = true;
  if (printToScroll_autoflush || linebreak) {
    //DEBUG("auto/line " + printToScroll_autoflush + SP + linebreak + SP);
    flushScroll();
  }
}
...
void clearScrollBuffer() {
    //DEBUG("clear 1 " + printToScroll_buffer);
  printToScroll_buffer = "";
  printToScroll_lastStyle = REGULAR;
    //DEBUG("clear 2 " + printToScroll_buffer);
}

void flushScroll() {
  //DEBUG("Flushing String Buffer: " + printToScroll_buffer + "Length: " + printToScroll_buffer.length());
...
String CWwordToClearText(String cwword) {             // decode the Morse code character in cwword to clear text
  ...
  //DEBUG("Symbol: " + symbol + " ptr: " + String(ptr));
  result += symbol;
    ...
    switch(decoderState) {
      case INTERELEMENT_: if (checkTone()) {
                              ...
                              if (lowDuration > (lacktime * ditAvg)) {
                                displayMorse(true);                                             /// decode the morse character and display it
...
void OFF_() {                                 /// what we do when we just detected a falling flank, from high to low
  ...
  //DEBUG("threshold: " + String(threshold));
...
void readPreferences(String repository) {
  ...
  /// new code for reading preferences values - we check if we have a value, and if yes, we use it; if no, we use and write a default value
    ...
    if ((temp = pref.getUChar("serialOut", 255)) != 255)
      ...
      pref.putUChar("serialOut", p_serialOut);
    ...
    if ((temp = pref.getUChar("lastExecuted")))
       p_menuPtr = temp;
    //DEBUG("read: p_menuPtr = " + String(p_menuPtr));
    ...
    p_maxSequence = pref.getUChar("maxSequence");
    ...
    if (atStart) {
     ...
     p_fileWordPointer = pref.getUInt("fileWordPtr"); // do not read fileWordPointer from other snapshots! we never write anything there!
    }  // endif atStart
   pref.end();
   updateTimings();
}
...
//DEBUG("Writing to repository: " + repository);
  ...
  pref.begin(repName, false);                // open namespace in read/write mode
    ...
    if (p_serialOut != pref.getUChar("serialOut"))
        pref.putUChar("serialOut", p_serialOut);
    ...
    if (! morserino)  {
        ...
        //DEBUG("write: last executed: " + String(p_menuPtr));
    }
