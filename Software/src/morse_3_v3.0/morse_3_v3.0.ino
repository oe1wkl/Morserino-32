...
ClickButton Buttons::modeButton(modeButtonPin);  // initialize mode button
...
//MorsePreferences::Koch::setup();
  
//        MorsePreferences::setKochChars(MorsePreferences::lcwoKochSeq);
//         
//        //// populate the array for abbreviations and words according to length and Koch filter
//        MorsePreferences::createKochWords(MorsePreferences::wordLength, MorsePreferences::kochFilter) ;  // 
//        MorsePreferences::createKochAbbr(MorsePreferences::abbrevLength, MorsePreferences::kochFilter);
  ...
  if (SPIFFS.begin() && digitalRead(modeButtonPin) == LOW)   {        // BLACK was pressed at start-up - checking for SPIFF so that programming 1st time w/o pull-up shows menu
     MorseOutput::clearDisplay();
     MorseOutput::printOnStatusLine( true, 0,  "Release BLACK");
     ...
     switch (hwConf) {
        case 1: MorsePreferences::calibrateVoltageMeasurement();
                break;
     ...
     if ((t = checkEncoder())) {
        ...
        switch (encoderState) {
          ...
          case volumeSettingMode: 
                                  MorsePreferences::sidetoneVolume += (t*10)+11;
                                  MorsePreferences::sidetoneVolume = constrain(MorsePreferences::sidetoneVolume, 11, 111) -11;
    ...
    case KEY_UP:
            ...
            if (l==0)  {                                               // fetch a new word if we have an empty word
                if (clearText.length() > 0) {                          // this should not be reached at all.... except when display word by word
                  ...
                  if (morseState == loraTrx || morseState == wifiTrx || (morseState == morseGenerator && MorsePreferences::trainerDisplay == DISPLAY_BY_WORD) ||
            ...
            if (morseState == morseGenerator && MorsePreferences::loraTrainerMode == 1)             // send the element to LoRa
...
void dispGeneratedChar() {
  ...
  if (generatorMode == KOCH_LEARN || morseState == loraTrx || morseState == wifiTrx || (morseState == morseGenerator && MorsePreferences::trainerDisplay == DISPLAY_BY_CHAR) ||
                    ( morseState == echoTrainer && MorsePreferences::echoDisplay != CODE_ONLY ))
...
void displayMorse(String symbol, boolean decoded) {
  ...
  //boolean flushe = morseState == morseGenerator ? !MorsePreferences::autoStopMode : true;
  //MorseOutput::printToScroll( REGULAR, symbol, flushe, encoderState == scrollMode);
...
void updateTopLine() {
  ...
  if ((morseState == loraTrx ) || (morseState == morseGenerator  && MorsePreferences::loraTrainerMode == true))
...
int16_t batteryVoltage() {      /// measure battery voltage and return result sin milliVolts
      ...
      // v *= (2314.35 / counts);
      v *= (MorsePreferences::vAdjust * 12.9 / counts);
