...
* **Can apps connect to the Morserino via Bluetooth (e.g. from an iPhone)?**
	* Yes: enable the preference **BLE Serial**. The device then offers the full **M32 Serial Protocol** over Bluetooth Low Energy (visible as "Morserino-32", no pairing needed), so apps can remote-control it and send text to be keyed as CW — the same protocol that is available over USB. See Appendix 8 of the user manual.
	* BLE Serial and the Bluetooth keyboard are mutually exclusive in CW Keyer mode: while BLE Serial is enabled and running, the keyboard output is skipped. Any WiFi function (multiplayer games, upload, update, WiFi transceiver) suspends the BLE connection until you return to the main menu.
