...
* **How to enable Bluetooth?**
	* Use the preference **Bluetooth Use**. The options besides **No Bluetooth** are four keyboard modes (VBand Kbd, Decoded output, VBand+Decoded, Generic Kbd) and **BLT Serial Prot.** (the M32 Serial Protocol over BLE, see below). 
...
* **Can apps connect to the Morserino via Bluetooth (e.g. from an iPhone)?**
	* Yes: set the preference **Bluetooth Use** to **BLT Serial Prot.** (takes effect on the next return to the main menu). The device then offers the full **M32 Serial Protocol** over Bluetooth Low Energy (visible as "Morserino-32", no pairing needed), so apps can remote-control it and send text to be keyed as CW — the same protocol that is available over USB. See Appendix 8 of the user manual.
