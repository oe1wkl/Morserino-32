...
IMPORTANT: In software version before 2.0 the WiFi functions were not integrated into the main menu. In case you want to update from version 1.x to version 2.x, please read section <<appendix3>> at the end of the document.
...
In CW Generator or Echo Trainer mode no characters will be output on USB.| **Keyer** / Decoder / Keyer+Decoder / ERRORS only
|===
...
The selectable options are "Calibr. Batt." (calibration of battery measurement), "LoRa Config." and "Cancel" (which just leaves this menu and continues with regular start-up iof the M32).
...
* Press the BLACK encoder knob to store the calibaration value, and to continue with the boot-up of the M32.
...
With firmware versions 1.x the WiFi functions were not accesible directly from the main menu, but ny quickly pressing the RED button three times. Hence the update procedure has to be performed as follows:
