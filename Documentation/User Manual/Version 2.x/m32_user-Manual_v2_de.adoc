...
Die vom Decoder erfasste aktuelle Geschwindigkeit wird als WpM in der Statuszeile angezeigt. Wann immer man den Decoder verlässt, bleibt diese Geschwindigkeitseinstellung erhalten - wenn man also zum CW Keyer wechselt, wird dieser auf den letzten mit dem Decoder erfassten Geschwindigkeitswert gesetzt. Ich bin mir nicht sicher, ob es sich hier um einen Bug oder um ein Feature handelt ;-)
