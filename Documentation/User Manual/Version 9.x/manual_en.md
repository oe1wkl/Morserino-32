...
| Boost Practice | Raises the odds that characters from your **Practice Set** (see **Preferences regarding Practice Set** above) come up when generating Random Characters groups, on top of the **Random Groups** subset selected above. Only affects Random Characters – Words, Abbreviations, Call Signs and the Koch Trainer are unaffected. Has no effect while Practice Set is empty; a Practice Set character that falls outside the current **Random Groups** subset (e.g. a digit while only Alpha is selected) is never boosted in on its own. | **Off** / Moderate / Strong |
