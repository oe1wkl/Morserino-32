...
-   Nur M32 Pocket, Accessibility Edition: Das Gerät sagt jetzt die neue Geschwindigkeit, sobald du sie änderst — die Zahl wird ausgesprochen, während du den Encoder drehst, damit ein blinder Operator sein Übungstempo selbst wählen kann, ohne jemanden fragen zu müssen. Drehst du den Encoder schnell, wird nur der Endwert gesagt: eine bereits laufende Ansage wird abgeschnitten, sobald der neuere Wert kommt, du hörst also nicht Anfangs- und Endwert mit allem dazwischen verschluckt. Der CW-Generator schweigt, solange die Ansage läuft (auf dem gemeinsamen Audiopfad würde CW sonst zu Klicken werden), und nimmt das nächste Element mit der neuen Geschwindigkeit auf. Es funktioniert überall dort, wo `changeSpeed()` zuständig ist, auch bei der automatischen Geschwindigkeitsanpassung des Echo Trainers nach jedem Wort.
...
Li-Ion-Zelle (3,7 V) mit Tiefentladeschutz.]{.pocket .pocket-a11y}
:::
...
| **Boost Practice** | Erhöht die Wahrscheinlichkeit, dass Zeichen aus deinem **Practice Set** (siehe **Einstellungen zum Practice Set** oben) vorkommen: bei Zufallszeichen-Gruppen zusätzlich zu der oben gewählten **Random Groups**-Teilmenge; und bei der Zeichenerzeugung des Koch Trainers (fest oder Adaptive) zusätzlich zu den Zeichen, die die aktuelle Koch-Lektion bereits freigeschaltet hat. Die eigene Gewichtung des Koch Trainers (das bevorzugte Üben neu gelernter bzw. adaptiver Zeichen) wird dabei nicht verändert – bei einem Fehltreffer wird Kochs eigenes Schema einfach erneut befragt, sodass dieses Verhalten exakt erhalten bleibt. Wirkt sich nur auf die Zeichenerzeugung aus – Wörter, Abkürzungen und Rufzeichen sind nicht betroffen. Hat keine Wirkung, solange kein Practice Set festgelegt ist; ein Practice-Set-Zeichen außerhalb des aktuell aktiven Zeichenvorrats (der **Random Groups**-Teilmenge, oder der aktuellen Koch-Lektion) wird dadurch nicht zusätzlich eingemischt. | **Off** / Moderate / Strong |
...
| []{.classic}LoRa | Long Range – eine Spreizspektrum-Funktechnologie für energiesparende Kommunikation über große Entfernungen. Wird in Morserinos der 1st und 2nd Edition für den drahtlosen CW-Transceiver-Modus verwendet. |
...
| []{.classic}Spreizspektrum | Eine Funktechnik, bei der das Signal über eine größere Bandbreite als für die Daten notwendig verteilt wird. LoRa nutzt dies, um bei geringer Leistung große Reichweiten zu erzielen. |
